...
$(document).ready(function(){
	...
	$(document).on("click","input:checkbox[name=com_small_category_no]",function(){	
		...
		if(bool){ // 체크박스에 클릭한 것이 체크된 것이라면 
			...
			var com_smcatgonoArr = document.querySelectorAll("input.com_small_category_no");
	...
	$(document).on("click","input:checkbox[name=my_small_category_no]",function(){	
		...
		if(bool){ // 체크박스에 클릭한 것이 체크된 것이라면 
			...
			var my_smcatgonoArr = document.querySelectorAll("input.my_small_category_no");
                	 ...
                	 */
                     ...
                     if(json.length > 0){
                             ...
                             $.each(json, function(index, item) {
                                    var schedule_startdate = moment(item.schedule_startdate).format('YYYY-MM-DD HH:mm:ss');
                                    var schedule_enddate = moment(item.schedule_enddate).format('YYYY-MM-DD HH:mm:ss');
                                    var schedule_subject = item.schedule_subject;
	                                		   ...
	                                		   if($("input:checkbox[name=com_small_category_no]:checked").eq(i).val() == item.fk_small_category_no){
	   			                               //  alert("캘린더 소분류 번호 : " + $("input:checkbox[name=com_smcatgono]:checked").eq(i).val());
	                                			   events.push({
	   			                                                ...
	   			                                                cid: item.fk_small_category_no  // 사내캘린더 내의 서브캘린더 체크박스의 value값과 일치하도록 만들어야 한다. 그래야만 서브캘린더의 체크박스와 cid 값이 연결되어 체크시 풀캘린더에서 일정이 보여지고 체크해제시 풀캘린더에서 일정이 숨겨져 안보이게 된다. 
	   			                                   }); // end of events.push({})---------
	   		                                   }
                                  ...
                                  if( $("input:checkbox[name=my_small_category_no]:checked").length <= $("input:checkbox[name=my_small_category_no]").length ){
	                                   ...
	                                   for(var i=0; i<$("input:checkbox[name=my_small_category_no]:checked").length; i++){
	                                		   ...
	                                		   if($("input:checkbox[name=my_small_category_no]:checked").eq(i).val() == item.fk_small_category_no && item.fk_member_userid == "${sessionScope.loginuser.member_userid}" ){
	   			                               //  alert("캘린더 소분류 번호 : " + $("input:checkbox[name=my_smcatgono]:checked").eq(i).val());
	                                			   events.push({
	   			                                        	    ...
	   			                                        	    url: "<%= ctxPath%>/schedule/detailSchedule?schedule_no="+item.schedule_no,
	   			                                                color: item.schedule_color,
	   			                                                cid: item.fk_small_category_no  // 내캘린더 내의 서브캘린더 체크박스의 value값과 일치하도록 만들어야 한다. 그래야만 서브캘린더의 체크박스와 cid 값이 연결되어 체크시 풀캘린더에서 일정이 보여지고 체크해제시 풀캘린더에서 일정이 숨겨져 안보이게 된다. 
	   			                                   }); // end of events.push({})---------
	   	                                    }
	                                   }// end of for-------------------------------------
...
function goAddComCal(){
	...
	if($("input.add_com_small_category_name").val().trim() == ""){
 		 ...
 		 $.ajax({
 			 ...
 			 type: "post",
 			 data: {"com_small_category_name": $("input.add_com_small_category_name").val(), 
 				    "fk_member_userid": "이순신"},
 			 dataType: "json",
 			 success:function(json){
 				 if(json.n != 1){
...
function goEditComCal(){
	...
	if($("input.edit_com_small_category_name").val().trim() == ""){
		...
		$.ajax({
			...
			type: "post",
			data:{"small_category_no":$("input.edit_com_small_category_no").val(), 
...
function goAddMyCal(){
	...
	if($("input.add_my_small_category_name").val().trim() == ""){
 		  ...
 		  $.ajax({
 			 ...
 			 type: "post",
 			 data: {"my_small_category_name": $("input.add_my_small_category_name").val(), 
 				    "fk_member_userid": "이순신"},
 			 dataType: "json",
 			 success:function(json){
	...
	$.ajax({
		 ...
		 success:function(json){
			 ...
			 if(json.length > 0){
				 ...
				 $.each(json, function(index, item){
					 html += "<tr style='font-size: 11pt;'>";
					 html += "<td style='width:60%; padding: 3px 0px;'><input type='checkbox' name='my_small_category_no' class='calendar_checkbox my_small_category_no' style='margin-right: 3px;' value='"+item.small_category_no+"' checked id='my_small_category_no_"+index+"' checked/><label for='my_small_category_no_"+index+"'>"+item.small_category_name+"</label></td>";   
...
function goEditMyCal(){
	...
	if($("input.edit_my_small_category_name").val().trim() == ""){
		 ...
		 $.ajax({
			url:"<%= ctxPath%>/schedule/editCalendar",
			type: "post",
			data:{"small_category_no":$("input.edit_my_small_category_no").val(), 
				  "small_category_name": $("input.edit_my_small_category_name").val(), 
				  "fk_member_userid":"${sessionScope.loginuser.fk_member_userid}",
				  "caltype":"1"  // 내캘린더
				  },
			dataType:"json",
			success:function(json){
...
function delCalendar(small_category_no, small_category_name){ // smcatgono => 캘린더 소분류 번호, smcatgoname => 캘린더 소분류 명
...
function goSearch(){
    ...
    frm.method="get";
    frm.action="<%= ctxPath%>/content/schedule/searchSchedule";
    frm.submit();
      ...
      <div class="modal-body">
       	...
       	<table style="width: 100%;" class="table table-bordered">
     			<tr>
     				...
     				<td><input type="text" class="add_com_small_category_name"/></td>
     			</tr>
     			<tr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       	...
       	<table style="width: 100%;" class="table table-bordered">
     			<tr>
     				<td style="text-align: left; ">소분류명</td>
     				<td><input type="text" class="edit_com_small_category_name"/><input type="hidden" value="" class="edit_com_small_category_no"></td>
     			</tr>
     			<tr>
     				<td style="text-align: left;">만든이</td>
     				<td style="text-align: left; padding-left: 5px;">이순신</td>
      ...
      <div class="modal-body">
      	<form name="modal_frm">
       	<table style="width: 100%;" class="table table-bordered">
     			<tr>
     				<td style="text-align: left; ">소분류명</td>
     				<td><input type="text" class="edit_my_small_category_name"/><input type="hidden" value="" class="edit_small_category_no"></td>
     			</tr>
     			<tr>
     				<td style="text-align: left;">만든이</td>
     				<td style="text-align: left; padding-left: 5px;">이순신</td>
