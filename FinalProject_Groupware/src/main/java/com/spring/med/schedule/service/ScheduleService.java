...
public interface ScheduleService {
	...
	List<Calendar_small_category_VO> showMyCalendar(String fk_member_userid);
...
//	List<MemberVO> searchJoinUserList(String joinUserName);
	...
	// 등록된 일정 가져오기
	List<Calendar_schedule_VO> selectSchedule(String fk_member_userid);

	// 일정 상세 보기 
	Map<String, String> detailSchedule(String schedule_no);

	// 일정삭제하기 
	int deleteSchedule(String schedule_no) throws Throwable;

	// 일정수정하기 
	int editSchedule_end(Calendar_schedule_VO svo) throws Throwable;

	// (사내캘린더 또는 내캘린더)속의  소분류 카테고리인 서브캘린더 삭제하기 
	int deleteSubCalendar(String small_category_no) throws Throwable;
