...
public class FileManager {
   ...
   // == 파일 업로드 하기 첫번째 방법 ==
    ...
    // 리턴값 : 서버에 저장된 새로운 파일명(예: 2025020709291535243254235235234.png => newFileName) 
   
   public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {  // 파일 경로가 잘못될 수도 있으니 Exception 처리를 해준다.
      
      String newFileName = null;
      // newFileName => ex) 2025020709291535243254235235234.png
      
      if(bytes == null) {  // 내용물이 없으면 나도 null
         return null;
      }
      
      // 클라이언트가 업로드한 파일의 이름
      if(originalFilename == null || "".equals(originalFilename)) { // 파일이름이 없다면 
         return null;
      }
      ...
      String fileExt = originalFilename.substring(originalFilename.lastIndexOf(".")); // "."부터 끝까지
      if(fileExt == null || "".equals(fileExt) || ".".equals(fileExt)) {    // 없거나  ""거나 . 일 때
         return null;
      }
      ...
      // newFileName 은 예를 들면, 2025020709291535243254235235234.png 와 같이 된다.
       ...
       // 업로드할 경로가 존재하지 않는 경우 폴더를 생성한다.(알아서 생성되어진다)
       ...
       if(!dir.exists()) {
          ...
          // mkdirs 서브 폴더까지 만들어줌, mkdir은 폴더만
   ...
   public boolean doFileDownload(String fileName, String originalFilename, String path, HttpServletResponse response) {
      ...
      // File.separator 은 운영체제에서 사용하는 파일경로의 구분자 이다.
        ...
        // 해당경로에 \ 를 더하고 파일명을 더한 경로까지 나타내어준 파일명(문자열)을 만든다. 
      // pathname은 예를 들면, C:\NCS\workspace_spring_boot_17\myspring\src\main\webapp\resources\files\202502071308391225872901291000.pdf 이다.
      
      
      try {
         if(originalFilename == null || "".equals(originalFilename) ) {
            originalFilename = fileName; // originalFilename가 없다면 originalFilename을 fileName로 바꾸겠다.
         }
   ...
   // === 파일 삭제하기 === //
   public void doFileDelete(String filename, String path) throws Exception { // 오류가 나면 메소드를 호출하는 쪽에서 Exception 처리해준다
