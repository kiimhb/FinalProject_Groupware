...
//	근태관리 - 근태현황 조회
	...
	@GetMapping("myLeave")
	public ModelAndView myLeave_count(HttpServletRequest request, HttpServletResponse response, ModelAndView mav,
									  @RequestParam(defaultValue = "") String searchType, 		
									  @RequestParam(defaultValue = "") String searchWord,
									  @RequestParam(defaultValue = "1") String currentShowPageNo) {
		...
		List<Map<String, String>> myLeave_count = service.get_myLeave_count(member_userid);
		//System.out.println(commute_count);
		mav.addObject("myLeave_count", myLeave_count);
		
		
		List<Map<String, String>> myLeave_list = null;
		...
		totalCount = service.get_myLeave_list_Total(paraMap);
		//System.out.println("~~~ 확인용 totalCount : " + totalCount);
		...
		myLeave_list = service.get_myLeave_list(paraMap);
		
		mav.addObject("myLeave_list", myLeave_list);
